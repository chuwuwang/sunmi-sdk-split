...
public class SaveKeyCipherTextUnderRsaActivity extends BaseAppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_security_savekey_ciphertext_under_rsa);
        initView();
    }

    private void initView() {
        initToolbarBringBack(R.string.security_save_ciphertext_key_under_rsa);
        ...
        findViewById(R.id.btn_ok).setOnClickListener((v) -> saveCiphertextKeyUnderRSA());
...
//        edtCheckValue.setText("B0B563C2");
    ...
    /** save a ciphertext mksk key, the key's decrypt key is a RSA private key */
    private void saveCiphertextKeyUnderRSA() {
        try {
            ...
            addStartTime("saveCiphertextKeyUnderRSA()");
            int code = MyApplication.app.securityOptV2.saveCiphertextKeyUnderRSA(keyIndex, rsaKeyIndex, keyType, keyAlgType, checkValue, keyData);
            addEndTime("saveCiphertextKeyUnderRSA()");
            String msg = null;
            if (code == 0) {
                msg = "saveCiphertextKeyUnderRSA() success";
            } else {
                msg = "saveCiphertextKeyUnderRSA() failed, code:" + code;
            }
            LogUtil.e(TAG, msg);
            showToast(msg);
        } catch (Exception e) {
