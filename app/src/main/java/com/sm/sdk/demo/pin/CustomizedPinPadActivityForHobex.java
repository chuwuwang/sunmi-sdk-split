...
/** P3H-GSensor to auto rotation 180° */
public class CustomizedPinPadActivityForHobex extends BaseAppCompatActivity implements SensorEventListener {
    ...
    private final String cardNo = "123456789123456";
    private SensorManager sensorManager;
    private Sensor accelerometer;
    private static final float ROTATION_THRESHOLD = 30.0f;
    private long lastRotationTime = 0;
    private static final long ROTATION_COOLDOWN = 1000; // 1秒冷却时间
    private volatile boolean rotateCancelPIN = false;
    private volatile int pinLen = 0;
    ...
    public void onSensorChanged(SensorEvent event) {
        if (pinLen == 0 && event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            float x = event.values[0];
            float y = event.values[1];
            float z = event.values[2];
            // 计算底部抬高的角度（假设设备正常持握，底部对应-Y方向）
            float bottomTitlAngle = (float) Math.toDegrees(Math.atan2(-y, Math.sqrt(x * x + z * z)));
            if (bottomTitlAngle < 0) {
                bottomTitlAngle += 360;
            }
            // 检查是否需要旋转屏幕（添加冷却时间）
            long currentTime = System.currentTimeMillis();
            if (currentTime - lastRotationTime > ROTATION_COOLDOWN) {
                checkRotationCondition(bottomTitlAngle);
            }
        }
    }

    @SuppressLint("SourceLockedOrientationActivity")
    private void checkRotationCondition(float currentBottomAngle) {
        boolean shouldBeUpsideDown = currentBottomAngle > ROTATION_THRESHOLD && currentBottomAngle < 180;
//        Log.e(TAG, "shouldBeUpsideDown->value:" + shouldBeUpsideDown);
        WindowManager manager = (WindowManager) getSystemService(Context.WINDOW_SERVICE);
        int rotation = manager.getDefaultDisplay().getRotation();
//        Log.e(TAG, "window->rotation:" + rotation);
        if (shouldBeUpsideDown && rotation != Surface.ROTATION_180) {//需要旋转180°，且当前旋转度数不是180°
            sensorManager.unregisterListener(this);
            rotateCancelPIN = true;
            cancelInputPIN();
            lastRotationTime = System.currentTimeMillis();
            reStartActivity(ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT);
        } else if (!shouldBeUpsideDown && rotation != Surface.ROTATION_0) {//需要旋转0°，且当前旋转度数不是0°
            sensorManager.unregisterListener(this);
            rotateCancelPIN = true;
            cancelInputPIN();
            lastRotationTime = System.currentTimeMillis();
            reStartActivity(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        }
    }
    ...
    private void handleOnCancel() {
        if (!isDestroyed() && !rotateCancelPIN) {
            showToast("CANCEL");
            finish();
        }
    }
...
//        data.keyMap[11] = 0x4B;//disable key( when press key, no number entered and no beep sound played)
    ...
    private void reStartActivity(int screenOrientation) {
        Intent intent = new Intent(this, CustomizedPinPadActivityForHobex.class);
