...
public final class DeviceUtil {
    ...
    public static boolean isD3Mini() {
        ...
        return model.matches("(d3mini|kuwait)(-.+)?");
    }
    ...
    public static boolean isBrazilCKD() {
        int value = SystemPropertiesUtil.getInt("ro.sunmi.special.customers", 0);
        String model = Build.MODEL.toLowerCase();
        return value == 1 || model.matches("(p2-b|p2mini-b).*");
    }
    ...
    public static boolean isP3() {
        String model = getModel();
        return model.matches("p3(-.+)?");
    }

    public static boolean isPayPad3() {
        String model = getModel();
        return model.matches("paypad3(-.+)?");
    }
    ...
    public static boolean isV3() {
        String model = getModel();
        return model.matches("(v3)(-.+)?");
    }

    /** 是否是V3_SE（SM11） */
    public static boolean isV3SE() {
        String model = getModel();
        return model.matches("(v3_se|v3 se)(-.+)?");
    }

    /** 是否是F2-H */
    public static boolean isF2H() {
        String model = getModelWithDeviceCode();
        return model.matches("(f2-h|t6f01)(-.+)?");
    }

    /** 是否是T3B00（蜻蜓） */
    public static boolean isT3B00() {
        String model = getModelWithDeviceCode();
        return !isF2H() && model.matches("(t3b00|f2|p371)(-.+)?"); // 蜻蜓T3B00
    }

    /** 是否是N8HU（CM01） */
    public static boolean isN8HU() {
        String model = getModel();
        return model.matches("n8hu(-.+)?");
    }

    /** 是否是L3(SM01) */
    public static boolean isL3() {
        String model = getModel();
        return model.matches("(l3)(-.+)?");
    }


    /** 是否是金融设备 */
    public static boolean isFinanceDevice() {
        boolean property = SystemPropertiesUtil.getBoolean("ro.sunmi.pay.device", false);
        boolean payDevice = isP1N() || isP14G() || isP2() || isP2Pro() || isP2Lite()
                || isP2Mini() || isP2SmartPad() || isP2XPro() || isP2Se() || isTossFront() || isTossTerminal()
                || isP2liteSE() || isCT621() || isP3Mix() || isP3();
        return property || payDevice;
    }

    /** 是否是非金融设备 */
    public static boolean isNPDevice() {
        return isFT2() || isFT2Mini() || isV2SE() || isV3Mix() || isF2H() || isT3B00() || isD3Mini() || isV3() || isN8HU() || isL3()
                || isV3SE();
    }
    ...
    private static String getModel() {
        ...
        String regex = "(p1n|p1_4g|p2|p2_pro|p2lite|p2mini|p2_smartpad|p2_retail|pinpad"
                + "|p2_xpro|p2h|p2_se|p2_lite_se|toss|ct621|p3_mix|p3|ft2|v2_se|v3_mix|paypad3).*";
