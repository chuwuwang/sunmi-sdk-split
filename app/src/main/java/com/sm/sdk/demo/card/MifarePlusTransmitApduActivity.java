...
import com.sm.sdk.demo.utils.CMacUtil;
...
//        followingAuthentication(block, 0, key);
    ...
    private void mifarePlusFirstAuthentication(int block, int keyType, byte[] key) {
        ...
        byte[] dec = DesAesUtil.aesDecrypt(key, new byte[16], enc, DesAesUtil.DATA_MODE_CBC);//plaintext data
        byte[] RndB = dec;//RndB
        ...
        byte[] temp = DesAesUtil.aesEncrypt(key, new byte[16], dataIn, DesAesUtil.DATA_MODE_CBC);
        ...
        dec = DesAesUtil.aesDecrypt(key, new byte[16], enc, DesAesUtil.DATA_MODE_CBC);//plaintext data
        ...
        showToast(msg);
        Log.e(TAG, msg);
        byte[] tmpA = DataOperateUtil.rightRotation(RdnA_, 1);
        ...
        byte[] E = DataOperateUtil.xor(C, D);
        // Session key base Kenc= A || B || E || 11h
        byte[] skbEnc = ByteUtil.concatByteArrays(A, B, E, new byte[]{0x11});
        Kenc = DesAesUtil.aesEncrypt(key, new byte[16], skbEnc, DesAesUtil.DATA_MODE_CBC);
        ...
        byte[] J = DataOperateUtil.xor(H, I);
        //Session key base KMac = F || G || J || 22h
        byte[] skbMac = ByteUtil.concatByteArrays(F, G, J, new byte[]{0x22});
        Kmac = DesAesUtil.aesEncrypt(key, new byte[16], skbMac, DesAesUtil.DATA_MODE_CBC);
    ...
    private void followingAuthentication(int block, int keyType, byte[] key) {
        ...
        byte[] RndB = DesAesUtil.aesDecrypt(key, rspIV, RndBEnc, DesAesUtil.DATA_MODE_CBC);//RndB
        ...
        byte[] RndB_ = DataOperateUtil.leftRotation(RndB, 1);
        byte[] dataIn = ByteUtil.concatByteArrays(RndA, RndB_);
        byte[] temp = DesAesUtil.aesEncrypt(key, cmdIV, dataIn, DesAesUtil.DATA_MODE_CBC);
        send = ByteUtil.concatByteArrays(new byte[]{cmd}, temp);
        recv = _transmitApdu(send);
        ...
        byte[] RdnA_ = DesAesUtil.aesDecrypt(key, rspIV, RdnA_Enc, DesAesUtil.DATA_MODE_CBC);//RndA'
        ...
        Kenc = DesAesUtil.aesEncrypt(key, new byte[16], skBase, DesAesUtil.DATA_MODE_CBC);
        ...
        Kmac = DesAesUtil.aesEncrypt(key, new byte[16], skMac, DesAesUtil.DATA_MODE_CBC);
...
//        byte[] blockData = aesEnDecrypt_CBC(Kenc, iv, blockDataEnc, Cipher.DECRYPT_MODE);
    ...
    private void mifarePlusWriteBlock(int block, byte[] dataIn) {
        ...
        byte[] dataInEnc = DesAesUtil.aesEncrypt(Kenc, iv, dataIn, DesAesUtil.DATA_MODE_CBC);
