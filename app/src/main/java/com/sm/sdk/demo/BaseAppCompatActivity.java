...
//        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
    ...
    public final void runOnUiThreadEx(Runnable action) {
        runOnUiThreadEx(action, 0L);
    }

    public final void runOnUiThreadEx(Runnable action, long delayMillis) {
        if (action == null || delayMillis < 0) {
            return;
        }
        if (Looper.myLooper() == Looper.getMainLooper() && delayMillis == 0) {//当前是UI线程
            action.run();
        } else { //当前不是UI线程
    ...
    private void showToastOnUI(final String msg) {
        runOnUiThreadEx(() -> Toast.makeText(this, msg, Toast.LENGTH_SHORT).show());
    }
    ...
    protected void showLoadingDialog(int resId) {
        runOnUiThreadEx(() -> _showLoadingDialog(getString(resId)));
    }

    protected void showLoadingDialog(final String msg) {
        runOnUiThreadEx(() -> _showLoadingDialog(msg));
    }
    ...
    protected void addTextViewText(CharSequence msg, TextView tv) {
        addTextViewText(msg, tv, true);
    }

    protected void addTextViewText(CharSequence msg, TextView tv, boolean showLog) {
        CharSequence tm = msg == null ? "" : msg;
        runOnUiThreadEx(() -> {
            CharSequence preMsg = tv.getText();
            if (TextUtils.isEmpty(preMsg)) {
                tv.setText(tm);
            } else {
