...
public class NFCActivity extends BaseAppCompatActivity {
    ...
    private TextView tvSak;
    private TextView tvAttrib;
    private TextView tvAtqa;
    private TextView tvAtqb;
    private View laySak;
    private View layAttrib;
    private View layAtqa;
    private View layAtqb;
    ...
    private void initView() {
        ...
        tvAttrib = findViewById(R.id.tv_attrib);
        tvAtqa = findViewById(R.id.tv_atqa);
        tvAtqb = findViewById(R.id.tv_atqb);
        laySak = findViewById(R.id.lay_sak);
        layAttrib = findViewById(R.id.lay_attrib);
        layAtqa = findViewById(R.id.lay_atqa);
        layAtqb = findViewById(R.id.lay_atqb);
    }
    ...
    private void handleResult(boolean success, Bundle info) {
        ...
        handler.post(() -> {
            ...
            if (success) {
                ...
                tvSak.setText(String.valueOf(info.getInt("sak")));
                tvAttrib.setText(String.valueOf(info.getInt("attrib")));
                tvAtqa.setText(ByteUtil.bytes2HexStr(info.getByteArray("atqa")));
                tvAtqb.setText(ByteUtil.bytes2HexStr(info.getByteArray("atqb")));
            } else {//on Error
                failCount++;
            ...
            if (info.getInt("cardCategory") == 'A') {
                laySak.setVisibility(View.VISIBLE);
                layAtqa.setVisibility(View.VISIBLE);
                layAttrib.setVisibility(View.GONE);
                layAtqb.setVisibility(View.GONE);
            } else if (info.getInt("cardCategory") == 'B') {
                laySak.setVisibility(View.GONE);
                layAtqa.setVisibility(View.GONE);
                layAttrib.setVisibility(View.VISIBLE);
                layAtqb.setVisibility(View.VISIBLE);
            }
            // 继续检卡
            if (!isFinishing()) {
