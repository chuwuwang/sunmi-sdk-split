...
public class VersionActivity extends BaseAppCompatActivity {
    ...
    private String getRomVersion() {
        String cfgMode = SystemPropertiesUtil.get("ro.fac.cfg.MODEL");
        String displayId = SystemPropertiesUtil.get("ro.build.display.id");
        String versionName = SystemPropertiesUtil.get("ro.version.sunmi_versionname");
        if (TextUtils.isEmpty(cfgMode)) {
            return versionName;
        } else if (!TextUtils.isEmpty(displayId)) {
            return displayId;
        } else if (!TextUtils.isEmpty(versionName)) {
            return versionName;
        }
        return getString(R.string.other_version_known);
    }
