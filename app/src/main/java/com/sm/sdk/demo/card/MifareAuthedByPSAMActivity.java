...
public class MifareAuthedByPSAMActivity extends BaseAppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_card_mifare_psam_test);
        initView();
    }
    ...
    private void checkCardPsam() {
        try {
            ...
            MyApplication.app.readCardOptV2.checkCard(CardType.PSAM0.getValue(), checkCardCallbackV2, 60);
    ...
    private void checkCardMifare() {
        try {
            result.setText(null);
            addStartTimeWithClear("checkCard()");
            MyApplication.app.readCardOptV2.checkCard(CardType.MIFARE.getValue(), checkCardCallbackV2, 60);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }

    private final CheckCardCallbackV2 checkCardCallbackV2 = new CheckCardCallbackV2Wrapper() {
...
//     如果不为0那么代表发送最后一个字节的若干位bit
            ...
            int len = MyApplication.app.readCardOptV2.transmitApduEx(CardType.MIFARE.getValue(), send, out);
            ...
            len = MyApplication.app.readCardOptV2.transmitApduEx(CardType.PSAM0.getValue(), send, out);
            ...
            send = ByteUtil.hexStr2Bytes(hexSend);
            addStartTimeWithClear("transmitApduEx() MIFARE");
            len = MyApplication.app.readCardOptV2.transmitApduEx(CardType.MIFARE.getValue(), send, out);
            addEndTime("transmitApduEx() MIFARE");
            if (len < 0) {
                String msg = "apdu交互出错，code:" + len;
                showResult(5, msg);
...
//     如果不为0那么代表发送最后一个字节的若干位bit
        try {
            ...
            byte[] out = new byte[128];
            addStartTimeWithClear("transmitApduEx() PSAM0");
            int len = MyApplication.app.readCardOptV2.transmitApduEx(CardType.PSAM0.getValue(), send, out);
            addEndTime("transmitApduEx() PSAM0");
            if (len < 0) {
                String msg = "apdu交互出错，code:" + len;
                showResult(1, msg);
...
//    2.将缓存区的值转存到目标block
    ...
    private void cancelCheckCard() {
        try {
            ...
            MyApplication.app.readCardOptV2.cardOff(CardType.PSAM0.getValue());
